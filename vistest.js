...
function mainContent(cityName = "中国") {
    d3.json(
        ...
        (error, root) => {
            ...
            const slice = g.selectAll('g.slice').data(partition(root).descendants().filter(d => d.x1 - d.x0 > 0.00005
            ));
            slice.exit().remove();
            ...
            const newSlice = slice
                ...
                .append('g')
                .attr('class', 'slice')
                // .on('mouseover',d =>changeOpacity(d))
                // .on('mouseleave',d => resetOpacity(d))
                .on('click', d => {
            ...
            function resetOpacity(d){
                newSlice.selectAll("path")
                    .transition()
                    .duration(1000)
                    .style("opacity", 1)
                    // 重新启用mouseover监听
                    .on("end", function() {
                        d3.select(this).on("mouseover", changeOpacity);
                    });
