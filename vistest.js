...
function mainContent(cityName = "中国") {
    d3.json(
        ...
        (error, root) => {
            ...
            const slice = g.selectAll('g.slice').data(partition(root).descendants());
            slice.exit().remove();
...
function showLeftContent() {
    d3.json('flare.json', (error, root) => {
        ...
        gRight.append('text').attr('id', "top-text-show")
            .datum(root)
            .attr('x', gRightStartX).attr('y', gRightStartY)
            .attr('font-size', 8.12)
            .attr('opacity', 1)
            .style('fill', "#646464")
            .text(d => show_time(d) + "(2020)")

        gRight.append('text').datum(root).attr('id', "top-text-show")
            .attr('x', gRightStartX).attr('y', gRightStartY+gRightpadding)
            .attr('font-size', 8.12)
            .attr('opacity', 1)
            .style('fill', "black")
            .text(d => '确诊省份: ' + d.children.length)
        ...
        gRight.append('text').attr('id', "top-text-show")
            ...
            .attr('opacity', 1)
            .style('fill', "black")
            .text('重点城市：' + maps[0].city_name + '、' + maps[1].city_name + '、' + maps[2].city_name)
