...
var threshold_copy = d3.scaleThreshold()//阈值比例尺
// .domain([0，max]) //这里咱们放在后面根据是否为武汉再定义数据的定义域
    .range([
        "#FFCC55" ,
        "#FFC521",
        ...
        (error, root) => {
           ...
           // 6.6 为每一块环的数据大小填充相应的颜色，并进行绘制环
            ...
            function city_color(d) {
                return d.children ? (d.value<1000?"#FFCC55":d.value<2000?
                    "#FFC521":d.value<4000?"#E89517":d.value<60000?"#FF7F25":d.value<80000?"#EB4E21":"#DB312B")
                    : day_color(d);
            }
...
function loadLegend(){
    ...
    var data = d3.range(100,600,100)
    console.log(data)
    var color = d3.scaleLinear()
        .domain(data)
        .range(threshold_copy.range());
    ...
    g_right.selectAll('.notwuhan').data(data).enter().append('rect')
        ...
        .attr("height",25)
        .attr("x",0)
        .attr("y",d => 230+Scale(d))
        .style('fill',d => color(d))
        .style('opacity',0.8)

    g_right.append('text').attr("x",60).attr("y",245).text('0').attr('class',"font-face")
    g_right.append('text').attr("x",60).attr("y",348).text('13,000+人确诊（武汉）').attr('class',"font-face")
    ...
    g_right.append('text').attr("x",-60).attr("y",296).text('100 to 125').attr('class',"font-face")
    g_right.append('text').attr("x",60).attr("y",296).text('600 to 800').attr('class',"font-face")
}
